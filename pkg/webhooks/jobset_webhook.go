...
func (j *jobSetWebhook) Default(ctx context.Context, obj runtime.Object) error {
	...
	if _, found := js.Labels[jobset.LabelManagedBy]; !found {
		if js.Labels == nil {
			js.Labels = make(map[string]string, 1)
		}
		js.Labels[jobset.LabelManagedBy] = jobset.JobSetManager
	}
	return nil
}
...
func (j *jobSetWebhook) ValidateUpdate(ctx context.Context, old, newObj runtime.Object) (admission.Warnings, error) {
	...
	errs = append(errs, apivalidation.ValidateImmutableField(js.Labels[jobset.LabelManagedBy], oldJS.Labels[jobset.LabelManagedBy], field.NewPath("metadata").Child("labels").Key(jobset.LabelManagedBy))...)
