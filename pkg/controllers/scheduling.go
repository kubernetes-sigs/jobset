...
// UseTopLevelGang returns true when the top-level scheduling model is used
// (Basic, Gang, or the default Gang policy) and there are no per-ReplicatedJob
// policy overrides.
...
func UseTopLevelGang(scheduling *jobset.JobSetScheduling) bool {
	...
	// Explicit Basic or Gang policy at the top level both use one top-level
	// PodGroup. The name is retained for compatibility with existing callers.
	return scheduling.SchedulingPolicy.Basic != nil || scheduling.SchedulingPolicy.Gang != nil
}
...
// totalMinCount computes the aggregate PodGroup minCount across all
// ReplicatedJobs. The JobSet-level policy is a composite policy and therefore
// does not expose a pod minCount; the controller derives it from the live
// represented pod count so scaling remains in sync.
...
func (r *JobSetReconciler) reconcileWorkload(ctx context.Context, js *jobset.JobSet) (bool, error) {
	...
	// Label the Workload consistently with its PodGroups so users and tools can
	// select all scheduling objects owned by a JobSet without relying on their
	// identity-hashed names.
	if desired.Labels == nil {
		desired.Labels = make(map[string]string)
	}
	desired.Labels[jobset.JobSetNameKey] = js.Name
...
// podGroupTemplatesEqualIgnoringMinCount reports whether the existing and
// desired PodGroupTemplates are equal, ignoring Gang.MinCount which is
// reconciled separately since it is the one field the upstream API allows to
// change after creation.
//
// SchedulingConstraints (TopologyAwareWorkloadScheduling) and ResourceClaims
// (DRAWorkloadResourceClaims) are feature-gated upstream and are silently
// dropped by the API server when their gate is off. Recreating the Workload
// cannot make them stick, so their absence from the persisted template is not
// treated as drift; otherwise a user who sets either field on a cluster
// without the gate would put the Workload into a permanent delete/recreate
// loop. See podGroupSpecsEqualIgnoringMinCount for the PodGroup equivalent.
func podGroupTemplatesEqualIgnoringMinCount(existing, desired *schedulingv1beta1.PodGroupTemplate) bool {
	e, d := existing.DeepCopy(), desired.DeepCopy()
	if e.SchedulingPolicy.Gang != nil {
		e.SchedulingPolicy.Gang.MinCount = 0
	}
	if d.SchedulingPolicy.Gang != nil {
		d.SchedulingPolicy.Gang.MinCount = 0
	}
	if e.SchedulingConstraints == nil {
		d.SchedulingConstraints = nil
	}
	if len(e.ResourceClaims) == 0 {
		d.ResourceClaims = nil
	}
	return apiequality.Semantic.DeepEqual(*e, *d)
}

// reconcilePodGroups ensures PodGroup resources exist for the JobSet using the
// workloadbuilder's NewPodGroup to materialize PodGroups from the compiled
// Workload. It reports whether it deleted the scheduling objects, in which case
// the caller must stop reconciling: child Jobs would otherwise be created
// pointing at PodGroups that no longer exist.
func (r *JobSetReconciler) reconcilePodGroups(ctx context.Context, js *jobset.JobSet) (bool, error) {
	log := ctrl.LoggerFrom(ctx)

	// Build the Workload to get the compiled templates.
	builders, err := newBuildersForJobSet(js)
	if err != nil {
		return false, fmt.Errorf("building Workload for PodGroup materialization: %w", err)
	}
	...
	for _, tmpl := range workload.Spec.PodGroupTemplates {
		...
		if err != nil {
			return false, fmt.Errorf("materializing PodGroup %s: %w", pgName, err)
		}
		...
		desired.OwnerReferences = nil
		if err := ctrl.SetControllerReference(js, desired, r.Scheme); err != nil {
			return false, fmt.Errorf("setting controller reference on PodGroup %s: %w", pgName, err)
		}
		// Label the PodGroup with the owning JobSet so deleteSchedulingObjects
		// can select just this JobSet's PodGroups instead of listing every
		// PodGroup in the namespace on each reconcile.
		if desired.Labels == nil {
			desired.Labels = make(map[string]string)
		}
		desired.Labels[jobset.JobSetNameKey] = js.Name
		...
		if apierrors.IsNotFound(getErr) {
			...
			if err := r.Create(ctx, desired); apierrors.IsAlreadyExists(err) {
				...
				return false, fmt.Errorf("creating PodGroup %s: %w", pgName, err)
			}
			continue
		}
		if getErr != nil {
			return false, fmt.Errorf("getting PodGroup %s: %w", pgName, getErr)
		}
		if !metav1.IsControlledBy(existing, js) {
			return false, fmt.Errorf("podgroup %s already exists and is not controlled by JobSet %s", klog.KObj(existing), klog.KObj(js))
		}
		...
		if existing.Spec.SchedulingPolicy.Gang != nil && desired.Spec.SchedulingPolicy.Gang != nil &&
			...
			if err := r.Update(ctx, patched); err != nil {
				return false, fmt.Errorf("patching PodGroup %s minCount: %w", pgName, err)
			}
			continue
		}
...
// field the upstream API allows to change after creation) and every field the
// API server, rather than this controller, decides the value of. Comparing
// those raw would see a server-side value as permanent drift and delete and
// recreate the scheduling objects on every reconcile, so the pods never
// gang-schedule.
//
// Two directions have to be normalized:
//
//   - Fields the server populates when we leave them unset: Priority and
//     PreemptionPolicy are always overwritten by the kube-apiserver's Priority
//     admission plugin from PriorityClassName, DisruptionMode defaults to
//     {Single: {}} via the structural schema, and an empty PriorityClassName is
//     replaced with the cluster's globalDefault PriorityClass if one exists.
//   - Fields the server drops when their feature gate is off:
//     SchedulingConstraints (TopologyAwareWorkloadScheduling) and
//     ResourceClaims (DRAWorkloadResourceClaims) come back unset no matter what
//     we send. Recreating the PodGroup cannot make them stick, so treating
//     their absence as drift only loops.
//
// Where the desired spec does set one of these explicitly and the server kept
// it, it is compared normally so real drift is still detected. PriorityClassName
// is the one field this costs us: a globalDefault is indistinguishable from a
// stale user-set class. That is not reachable through the JobSet API, since the
// pod template it is derived from is immutable after creation.
func podGroupSpecsEqualIgnoringMinCount(existing, desired schedulingv1beta1.PodGroupSpec) bool {
	...
	if desired.PreemptionPolicy == nil {
		existing.PreemptionPolicy = nil
	}
	if desired.DisruptionMode == nil {
		existing.DisruptionMode = nil
	}
	if desired.PriorityClassName == "" {
		existing.PriorityClassName = ""
	}
	if existing.SchedulingConstraints == nil {
		desired.SchedulingConstraints = nil
	}
	if len(existing.ResourceClaims) == 0 {
		desired.ResourceClaims = nil
	}
...
func (r *JobSetReconciler) deleteSchedulingObjects(ctx context.Context, js *jobset.JobSet) error {
	...
	// Delete PodGroups owned by this JobSet. This runs on every reconcile of a
	// suspended or finished JobSet, so select on the owning-JobSet label rather
	// than listing every PodGroup in the namespace. The controller ref is still
	// checked below, since the label alone does not prove ownership.
	var pgList schedulingv1beta1.PodGroupList
	if err := r.List(ctx, &pgList, client.InNamespace(js.Namespace),
		client.MatchingLabels{jobset.JobSetNameKey: js.Name}); err != nil {
...
// schedulingJobsNeedRecreation reports whether active child Jobs still point at
// scheduling objects compiled from an older JobSet scheduling configuration.
// Job pod templates are immutable, so changing scheduling while suspended
// requires these Jobs to be deleted and recreated before they can run again.
func schedulingJobsNeedRecreation(js *jobset.JobSet, jobs []*batchv1.Job) bool {
	for _, job := range jobs {
		...
		templateName := schedulingGroupTemplateName(js, rjobName, jobIdx)
		if job.Annotations == nil || job.Annotations[SchedulingGroupTemplateNameKey] != templateName {
			return true
		}
		// pod.spec.schedulingGroup is gated upstream by GenericWorkload. When
		// that gate is off on the API server the field is stripped from every
		// Job we create, so a nil value here says nothing about which
		// configuration the Job was built from — deleting and recreating would
		// produce another stripped Job and loop forever. The annotation above,
		// which is never stripped, already identifies the configuration; only
		// compare the PodGroup name when the server actually persisted it.
		sg := job.Spec.Template.Spec.SchedulingGroup
		if sg == nil {
			continue
		}
		expectedPodGroupName := schedulingPodGroupName(js, templateName)
		if sg.PodGroupName == nil || *sg.PodGroupName != expectedPodGroupName {
			return true
		}
	}
	return false
