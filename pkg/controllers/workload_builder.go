...
// mapCompositePolicyInput maps a composite JobSet or ReplicatedJob policy to
// the leaf PodGroup policy used by the alpha implementation. Composite groups
// are not materialized yet, so only the policy variant is carried over; a gang's
// PodGroup minCount is derived from the represented pods by the controller.
func mapCompositePolicyInput(policy *schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy, pathElements []string) workloadbuilder.PolicyInput {
	if policy == nil {
...
// mapPodPolicyInput maps an individual Job's leaf policy directly to the
// workloadbuilder input.
func mapPodPolicyInput(policy *schedulingv1alpha3.WorkloadPodGroupSchedulingPolicy, pathElements []string) workloadbuilder.PolicyInput {
	if policy == nil {
		return workloadbuilder.PolicyInput{}
	}
	return workloadbuilder.PolicyInput{
		PodGroupData: policy.DeepCopy(),
		PathElements: pathElements,
	}
}

// mapCompositeConstraintsInput maps composite constraints to the leaf PodGroup
// constraints used by the alpha implementation, which does not materialize
// CompositePodGroups yet.
func mapCompositeConstraintsInput(constraints *schedulingv1alpha3.WorkloadCompositePodGroupSchedulingConstraints, pathElements []string) workloadbuilder.ConstraintsInput {
	if constraints == nil {
		return workloadbuilder.ConstraintsInput{}
	}
	return workloadbuilder.ConstraintsInput{
		PodGroupData: &schedulingv1alpha3.WorkloadPodGroupSchedulingConstraints{
			Topology: append([]schedulingv1alpha3.TopologyConstraint(nil), constraints.Topology...),
		},
		PathElements: pathElements,
	}
}

// mapPodConstraintsInput maps an individual Job's leaf constraints directly
// to the workloadbuilder input.
func mapPodConstraintsInput(constraints *schedulingv1alpha3.WorkloadPodGroupSchedulingConstraints, pathElements []string) workloadbuilder.ConstraintsInput {
	if constraints == nil {
		return workloadbuilder.ConstraintsInput{}
	}
	return workloadbuilder.ConstraintsInput{
		PodGroupData: constraints.DeepCopy(),
		PathElements: pathElements,
	}
}

// mapCompositeDisruptionInput maps a composite disruption mode to the leaf
// PodGroup mode used by the alpha implementation.
func mapCompositeDisruptionInput(disruption *schedulingv1alpha3.WorkloadCompositePodGroupDisruptionMode, pathElements []string) workloadbuilder.DisruptionModeInput {
	if disruption == nil {
...
// mapPodDisruptionInput maps an individual Job's leaf disruption mode directly
// to the workloadbuilder input.
func mapPodDisruptionInput(disruption *schedulingv1alpha3.WorkloadPodGroupDisruptionMode, pathElements []string) workloadbuilder.DisruptionModeInput {
	if disruption == nil {
		return workloadbuilder.DisruptionModeInput{}
	}
	return workloadbuilder.DisruptionModeInput{
		PodGroupData: disruption.DeepCopy(),
		PathElements: pathElements,
	}
}

// mapResourceClaimsInput maps workload resource claims to the workloadbuilder input.
func mapResourceClaimsInput(claims []schedulingv1alpha3.WorkloadPodGroupResourceClaim, pathElements []string) workloadbuilder.ResourceClaimsInput {
	if len(claims) == 0 {
		return workloadbuilder.ResourceClaimsInput{}
	}
	wlClaims := make([]schedulingv1alpha3.WorkloadPodGroupResourceClaim, len(claims))
	for i := range claims {
		claims[i].DeepCopyInto(&wlClaims[i])
	}
	return workloadbuilder.ResourceClaimsInput{
...
func buildTopLevelGangItem(js *jobset.JobSet) *workloadbuilder.WorkloadItem {
	...
	input := workloadbuilder.WorkloadInput{
		Policy:      mapCompositePolicyInput(policy, []string{"schedulingPolicy"}),
		Constraints: mapCompositeConstraintsInput(scheduling.SchedulingConstraints, []string{"schedulingConstraints"}),
	}
	if scheduling.DisruptionMode != nil {
		input.DisruptionMode = mapCompositeDisruptionInput(scheduling.DisruptionMode, []string{"disruptionMode"})
	}
...
// configuration.
func buildPerRJItem(js *jobset.JobSet, rjob *jobset.ReplicatedJob, globalScheduling *jobset.JobSetScheduling) *workloadbuilder.WorkloadItem {
	input := globalSchedulingInput(globalScheduling)
...
func buildGroupItem(js *jobset.JobSet, members []*jobset.ReplicatedJob, leafPolicy *jobset.ReplicatedJobScheduling, globalScheduling *jobset.JobSetScheduling) *workloadbuilder.WorkloadItem {
	...
	if leafSchedulingPolicy != nil {
		input.Policy = mapCompositePolicyInput(leafSchedulingPolicy, []string{"replicatedJobs", groupName, "schedulingPolicy"})
	}
	if leafPolicy.SchedulingConstraints != nil {
		input.Constraints = mapCompositeConstraintsInput(leafPolicy.SchedulingConstraints, []string{"replicatedJobs", groupName, "schedulingConstraints"})
	}
	if leafPolicy.DisruptionMode != nil {
		input.DisruptionMode = mapCompositeDisruptionInput(leafPolicy.DisruptionMode, []string{"replicatedJobs", groupName, "disruptionMode"})
	}

	// If the leaf policy didn't set policy/constraints/disruption, fall back to global.
	globalInput := globalSchedulingInput(globalScheduling)
...
func buildJobItem(js *jobset.JobSet, rjob *jobset.ReplicatedJob, jobIdx int, jobPolicy *jobset.JobScheduling, globalScheduling *jobset.JobSetScheduling) *workloadbuilder.WorkloadItem {
	...
	var input workloadbuilder.WorkloadInput
	if policy != nil {
		input.Policy = mapPodPolicyInput(policy, []string{"replicatedJobs", rjob.Name, "job", "schedulingPolicy"})
	}
	if jobPolicy.SchedulingConstraints != nil {
		input.Constraints = mapPodConstraintsInput(jobPolicy.SchedulingConstraints, []string{"replicatedJobs", rjob.Name, "job", "schedulingConstraints"})
	}
	if jobPolicy.DisruptionMode != nil {
		input.DisruptionMode = mapPodDisruptionInput(jobPolicy.DisruptionMode, []string{"replicatedJobs", rjob.Name, "job", "disruptionMode"})
	}
	...
	// Fall back to the JobSet's global scheduling policy, constraints, and
	// disruption mode when job didn't set them, matching the leaf-level
	// fallback behavior used for grouped/per-RJ PodGroups.
	globalInput := globalSchedulingInput(globalScheduling)
	if input.Policy.PodGroupData == nil {
		input.Policy = globalInput.Policy
	}
...
func globalSchedulingInput(globalScheduling *jobset.JobSetScheduling) workloadbuilder.WorkloadInput {
	var input workloadbuilder.WorkloadInput
	if globalScheduling == nil {
		return input
	}
	if globalScheduling.SchedulingPolicy != nil {
		input.Policy = mapCompositePolicyInput(globalScheduling.SchedulingPolicy, []string{"schedulingPolicy"})
	}
	if globalScheduling.SchedulingConstraints != nil {
		input.Constraints = mapCompositeConstraintsInput(globalScheduling.SchedulingConstraints, []string{"schedulingConstraints"})
	}
	if globalScheduling.DisruptionMode != nil {
		input.DisruptionMode = mapCompositeDisruptionInput(globalScheduling.DisruptionMode, []string{"disruptionMode"})
	}
	return input
}
...
func newBuildersForJobSet(js *jobset.JobSet) ([]*workloadbuilder.Builder, error) {
	...
	if scheduling != nil {
		for i := range scheduling.ReplicatedJobs {
			...
			item := buildGroupItem(js, members, policy, scheduling)
			if err := addBuilder(item); err != nil {
				return nil, err
			}
		}
	...
	for i := range js.Spec.ReplicatedJobs {
		...
		item := buildPerRJItem(js, rjob, scheduling)
		if err := addBuilder(item); err != nil {
			return nil, err
		}
	}
...
func buildWorkloadFromBuilders(builders []*workloadbuilder.Builder) (*schedulingv1beta1.Workload, error) {
	if len(builders) == 0 {
		return nil, errors.New("no scheduling items were compiled from spec.scheduling")
	}
	if len(builders) == 1 {
