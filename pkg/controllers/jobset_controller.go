...
func (r *JobSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if jobSetFinished(&js) {
		if js.Spec.TTLSecondsAfterFinished != nil {
			expiredAt, err := r.checkIfTTLExpired(ctx, &js)
			if err != nil {
				log.Error(err, "checking if TTL expired")
				return ctrl.Result{}, err
			}
			// if expiredAt is not nil, that means the TTL has expired, and we should delete the JobSet
			// otherwise, we requeue it for the remaining TTL duration.
			if expiredAt != nil {
				log.V(5).Info("JobSet TTL expired", "expiredAt", *expiredAt)
...
func (r *JobSetReconciler) checkIfTTLExpired(ctx context.Context, jobSet *jobset.JobSet) (*time.Time, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	now := r.clock.Now()
	remaining, expiredAt, err := timeLeft(log, jobSet, &now)
	if err != nil {
		return nil, err
	}
...
func timeLeft(log logr.Logger, js *jobset.JobSet, since *time.Time) (*time.Duration, *time.Time, error) {
	...
	if since == nil {
		log.V(2).Info("Warning: Received invalid since time. JobSet cleanup will be deferred.")
		return nil, nil, nil
	}
	if finishAt == nil || expireAt == nil {
		log.V(2).Info("Warning: Received invalid finish or expire time. JobSet cleanup will be deferred.")
		return nil, nil, nil
	}
	...
	log.V(2).Info("Found JobSet finished", "finishTime", finishAt.UTC(), "remainingTTL", remaining, "startTime", since.UTC(), "deadlineTTL", expireAt.UTC())
