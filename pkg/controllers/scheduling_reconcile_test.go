...
func TestReconcileWorkloadLabelsCreatedWorkload(t *testing.T) {
	js := schedulingTestJobSet()
	scheme := schedulingTestScheme()
	fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
	r := &JobSetReconciler{Client: fakeClient, Scheme: scheme}

	deleted, err := r.reconcileWorkload(context.Background(), js)
	if err != nil {
		t.Fatalf("reconcileWorkload() error = %v", err)
	}
	if deleted {
		t.Fatal("reconcileWorkload() reported deletion for a newly created Workload")
	}
...
func TestReconcilePodGroupsDeletesOwnedStaleObjects(t *testing.T) {
	...
	// from the (hashed) PodGroupTemplate name, so reconcilePodGroups finds it.
	...
	r := &JobSetReconciler{Client: fakeClient, Scheme: scheme}
	deleted, err := r.reconcilePodGroups(context.Background(), js)
	if err != nil {
		t.Fatalf("reconcilePodGroups() error = %v", err)
	}
	if !deleted {
		t.Fatal("reconcilePodGroups() reported no deletion, want true so the caller stops before creating Jobs that reference the deleted PodGroup")
	}
