...
func TestPolicyResolutionThroughBuildWorkload(t *testing.T) {
	tests := map[string]struct {
		...
		"global composite gang derives pod minCount": {
			js: &jobset.JobSet{
				ObjectMeta: metav1.ObjectMeta{Name: "test", Namespace: "default"},
				Spec: jobset.JobSetSpec{
					Scheduling: &jobset.JobSetScheduling{
						SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
							Gang: &schedulingv1alpha3.WorkloadCompositePodGroupGangSchedulingPolicy{MinGroupCount: ptr.To[int32](5)},
						},
					},
					ReplicatedJobs: []jobset.ReplicatedJob{
						{Name: "workers", Replicas: 4, Template: batchv1.JobTemplateSpec{
		...
		"sequenced startup fallback ignores explicit global gang minCount": {
			js: &jobset.JobSet{
				...
				Spec: jobset.JobSetSpec{
					...
					Scheduling: &jobset.JobSetScheduling{
						SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
							Gang: &schedulingv1alpha3.WorkloadCompositePodGroupGangSchedulingPolicy{MinGroupCount: ptr.To[int32](99)},
						},
					},
					ReplicatedJobs: []jobset.ReplicatedJob{
		...
		"leaf override takes priority over global": {
			js: &jobset.JobSet{
				...
				Spec: jobset.JobSetSpec{
					Scheduling: &jobset.JobSetScheduling{
						SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
							Gang: &schedulingv1alpha3.WorkloadCompositePodGroupGangSchedulingPolicy{MinGroupCount: ptr.To[int32](10)},
						},
						ReplicatedJobs: []jobset.ReplicatedJobScheduling{
							{
								TargetReplicatedJobs: []string{"driver"},
		...
		"composite Gang minGroupCount does not override derived pod minCount": {
			js: &jobset.JobSet{
				ObjectMeta: metav1.ObjectMeta{Name: "test", Namespace: "default"},
				Spec: jobset.JobSetSpec{
					Scheduling: &jobset.JobSetScheduling{
						ReplicatedJobs: []jobset.ReplicatedJobScheduling{
							{
								...
								SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
									Gang: &schedulingv1alpha3.WorkloadCompositePodGroupGangSchedulingPolicy{MinGroupCount: ptr.To[int32](16)},
								},
							},
						},
					},
						...
						ReplicatedJobs: []jobset.ReplicatedJobScheduling{
							{
								...
								DisruptionMode: &schedulingv1alpha3.WorkloadCompositePodGroupDisruptionMode{All: &schedulingv1alpha3.WorkloadCompositePodGroupAllDisruptionMode{}},
							},
						},
					},
					ReplicatedJobs: []jobset.ReplicatedJob{{Name: "workers", Replicas: 2, Template: batchv1.JobTemplateSpec{Spec: batchv1.JobSpec{Parallelism: ptr.To[int32](4)}}}},
...
func TestBuildWorkloadJobSchedulingIgnoresGlobalGangMinCount(t *testing.T) {
	...
	for _, tmpl := range workload.Spec.PodGroupTemplates {
		...
		case groupTemplateName(js, []string{"launcher"}):
			require.NotNil(t, tmpl.SchedulingPolicy.Gang)
			// Composite minGroupCount is not a pod count, so the flattened
			// launcher PodGroup derives minCount from its represented pods.
			assert.Equal(t, int32(1), tmpl.SchedulingPolicy.Gang.MinCount)
...
func TestSchedulingJobsNeedRecreation(t *testing.T) {
	js := &jobset.JobSet{
		ObjectMeta: metav1.ObjectMeta{Name: "js", Namespace: "default"},
		Spec: jobset.JobSetSpec{
			Scheduling:     &jobset.JobSetScheduling{},
	...
	templateName := schedulingGroupTemplateName(js, "workers", 0)
	podGroupName := schedulingPodGroupName(js, templateName)
	job := &batchv1.Job{ObjectMeta: metav1.ObjectMeta{
		Labels: map[string]string{
			jobset.ReplicatedJobNameKey: "workers",
			jobset.JobIndexKey:          "0",
		},
		Annotations: map[string]string{SchedulingGroupTemplateNameKey: templateName},
	}, Spec: batchv1.JobSpec{Template: corev1.PodTemplateSpec{Spec: corev1.PodSpec{
		SchedulingGroup: &corev1.PodSchedulingGroup{PodGroupName: ptr.To(podGroupName)},
	}}}}

	assert.False(t, schedulingJobsNeedRecreation(js, []*batchv1.Job{job}))
	job.Annotations[SchedulingGroupTemplateNameKey] = "stale"
	assert.True(t, schedulingJobsNeedRecreation(js, []*batchv1.Job{job}))
}

func TestTopLevelBasicBuildsSingleTemplate(t *testing.T) {
	js := &jobset.JobSet{
		ObjectMeta: metav1.ObjectMeta{Name: "basic-js", Namespace: "default"},
		Spec: jobset.JobSetSpec{
			Scheduling: &jobset.JobSetScheduling{
				SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
					Basic: &schedulingv1alpha3.WorkloadCompositePodGroupBasicSchedulingPolicy{},
			...
			ReplicatedJobs: []jobset.ReplicatedJob{
				...
				{Name: "worker", Replicas: 2, Template: batchv1.JobTemplateSpec{Spec: batchv1.JobSpec{Parallelism: ptr.To[int32](2)}}},
			},
		},
	}
	...
	assert.NotNil(t, workload.Spec.PodGroupTemplates[0].SchedulingPolicy.Basic)
}
...
func TestTotalMinCount(t *testing.T) {
	tests := map[string]struct {
		...
		"composite minGroupCount does not override total pod count": {
			js: &jobset.JobSet{
				Spec: jobset.JobSetSpec{
					Scheduling: &jobset.JobSetScheduling{
						SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
...
func TestBuildTopLevelGangThroughBuildWorkload(t *testing.T) {
	tests := map[string]struct {
		...
		"composite minGroupCount does not override derived pod minCount": {
			js: &jobset.JobSet{
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.NotNil(t, template.SchedulingPolicy.Gang)
			assert.Equal(t, tc.wantMinCount, template.SchedulingPolicy.Gang.MinCount)
			assert.Empty(t, template.ResourceClaims)
			if tc.hasConstraints {
				require.NotNil(t, template.SchedulingConstraints)
			} else {
				assert.Nil(t, template.SchedulingConstraints)
...
// TestPodGroupSpecsEqualIgnoringServerManagedFields covers the fields the API
// server, not this controller, decides the value of. Treating any of them as
// drift would put reconcilePodGroups into a permanent delete/recreate loop.
func TestPodGroupSpecsEqualIgnoringServerManagedFields(t *testing.T) {
	gang := func() schedulingv1beta1.PodGroupSchedulingPolicy {
		return schedulingv1beta1.PodGroupSchedulingPolicy{
			Gang: &schedulingv1beta1.GangSchedulingPolicy{MinCount: 4},
		}
	}
	constraints := &schedulingv1beta1.PodGroupSchedulingConstraints{
		Topology: []schedulingv1beta1.TopologyConstraint{{Key: "topology.kubernetes.io/rack"}},
	}

	tests := map[string]struct {
		existing schedulingv1beta1.PodGroupSpec
		desired  schedulingv1beta1.PodGroupSpec
		want     bool
	}{
		"globalDefault priorityClassName injected by admission is not drift": {
			existing: schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang(), PriorityClassName: "cluster-default"},
			desired:  schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang()},
			want:     true,
		},
		"priority populated by the Priority admission plugin is not drift": {
			existing: schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang(), Priority: ptr.To[int32](0)},
			desired:  schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang()},
			want:     true,
		},
		"preemptionPolicy populated by the Priority admission plugin is not drift": {
			existing: schedulingv1beta1.PodGroupSpec{
				SchedulingPolicy: gang(),
				PreemptionPolicy: ptr.To(schedulingv1beta1.PreemptLowerPriority),
			},
			desired: schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang()},
			want:    true,
		},
		"defaulted disruptionMode is not drift": {
			existing: schedulingv1beta1.PodGroupSpec{
				SchedulingPolicy: gang(),
		...
		"schedulingConstraints stripped by a disabled feature gate is not drift": {
			existing: schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang()},
			desired:  schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang(), SchedulingConstraints: constraints},
			want:     true,
		},
		"resourceClaims stripped by a disabled feature gate is not drift": {
			existing: schedulingv1beta1.PodGroupSpec{SchedulingPolicy: gang()},
			desired: schedulingv1beta1.PodGroupSpec{
				SchedulingPolicy: gang(),
				ResourceClaims:   []schedulingv1beta1.PodGroupResourceClaim{{Name: "gpus"}},
			},
			want: true,
		},
		"schedulingConstraints the server kept but that no longer match is drift": {
			existing: schedulingv1beta1.PodGroupSpec{
				SchedulingPolicy: gang(),
		...
		"a changed scheduling policy is still drift": {
			existing: schedulingv1beta1.PodGroupSpec{
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			assert.Equal(t, tc.want, podGroupSpecsEqualIgnoringMinCount(tc.existing, tc.desired))
		})
	}
}

// TestWorkloadNeedsRecreationWithStrippedGatedFields covers PodGroupTemplate
// fields the API server drops when their upstream feature gate is off.
// Recreating the Workload cannot make them stick, so their absence must not be
// reported as drift.
func TestWorkloadNeedsRecreationWithStrippedGatedFields(t *testing.T) {
	workload := func(tmpl schedulingv1beta1.PodGroupTemplate) *schedulingv1beta1.Workload {
		return &schedulingv1beta1.Workload{
			Spec: schedulingv1beta1.WorkloadSpec{PodGroupTemplates: []schedulingv1beta1.PodGroupTemplate{tmpl}},
		}
	}
	...
	existing := workload(schedulingv1beta1.PodGroupTemplate{Name: "js", SchedulingPolicy: policy})
	desired := workload(schedulingv1beta1.PodGroupTemplate{
		Name:             "js",
	...
	assert.False(t, workloadNeedsRecreation(existing, desired))

	// A template the server did persist is still compared normally.
	existing.Spec.PodGroupTemplates[0].SchedulingConstraints = &schedulingv1beta1.PodGroupSchedulingConstraints{
		Topology: []schedulingv1beta1.TopologyConstraint{{Key: "topology.kubernetes.io/zone"}},
	}
	assert.True(t, workloadNeedsRecreation(existing, desired))
}

// TestSchedulingJobsNeedRecreationWithStrippedSchedulingGroup covers a cluster
// where the upstream GenericWorkload gate is off and pod.spec.schedulingGroup is
// stripped from every Job we create. Reporting that as drift would delete and
// recreate the same Jobs forever without ever making progress.
func TestSchedulingJobsNeedRecreationWithStrippedSchedulingGroup(t *testing.T) {
	js := &jobset.JobSet{
		ObjectMeta: metav1.ObjectMeta{Name: "js", Namespace: "default"},
		Spec: jobset.JobSetSpec{
			Scheduling:     &jobset.JobSetScheduling{},
	...
	// The annotation is never stripped, so a genuine scheduling change is still
	// detected on such a cluster.
	job.Annotations[SchedulingGroupTemplateNameKey] = "stale"
	assert.True(t, schedulingJobsNeedRecreation(js, []*batchv1.Job{job}))
}

// TestBuildWorkloadFromBuildersRejectsEmptyBuilders guards the merge path
// against an empty builder slice, which newBuildersForJobSet returns when
// per-ReplicatedJob mode yields no items.
func TestBuildWorkloadFromBuildersRejectsEmptyBuilders(t *testing.T) {
	wl, err := buildWorkloadFromBuilders(nil)
	require.Error(t, err)
	assert.Nil(t, wl)
}
