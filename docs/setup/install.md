...
- Your cluster has at least 1 node with 2+ CPUs and 512+ MB of memory available for the JobSet controller manager Deployment to run on. **NOTE**: On some cloud providers, the default node machine type will not have sufficient resources and you will need to specify a larger machine type.
