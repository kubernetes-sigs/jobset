...
var _ = ginkgo.Describe("Workload-Aware Scheduling integration", func() {
	...
	ginkgo.It("should create Workload and per-RJ PodGroups when leaf overrides are present", func() {
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			// The composite gang policy's minGroupCount counts child groups, not
			// pods, so the controller derives the PodGroup minCount from the
			// represented pods: 4 replicas x parallelism 2.
			g.Expect(workersPG.Spec.SchedulingPolicy.Gang.MinCount).To(gomega.Equal(int32(8)))
		}, timeout, interval).Should(gomega.Succeed())
	})
	...
	ginkgo.It("should create PodGroups with Basic policy when overridden per ReplicatedJob", func() {
		...
		gomega.Eventually(func(g gomega.Gomega) {
			workersPG := getPodGroupByPrefix(ctx, g, nsObj.Name, fmt.Sprintf("%s-workers", js.Name))
			g.Expect(workersPG.Spec.SchedulingPolicy.Gang).NotTo(gomega.BeNil())
			// minGroupCount counts child groups, not pods; the PodGroup minCount is
			// derived from the represented pods (4 replicas x parallelism 2).
			g.Expect(workersPG.Spec.SchedulingPolicy.Gang.MinCount).To(gomega.Equal(int32(8)))
	...
	ginkgo.It("should create a single Basic PodGroup for the whole JobSet when the top-level policy is Basic", func() {
		// Top-level mode is selected whenever startup is not sequenced and there
		// are no replicatedJobs overrides, regardless of whether the effective
		// composite policy is Gang or Basic (see the KEP's Controller Integration
		// section). One WorkloadItem therefore represents the entire JobSet; with
		// a Basic policy its PodGroup carries no minCount and each pod is
		...
		ginkgo.By("verifying the Workload has a single JobSet-wide Basic PodGroupTemplate")
		gomega.Eventually(func(g gomega.Gomega) {
			workload := getWorkloadForJobSet(ctx, g, nsObj.Name)
			g.Expect(workload.Spec.PodGroupTemplates).To(gomega.HaveLen(1))
			tmpl := workload.Spec.PodGroupTemplates[0]
			g.Expect(tmpl.Name).To(gomega.HavePrefix(fmt.Sprintf("%s-", js.Name)))
			g.Expect(tmpl.SchedulingPolicy.Basic).NotTo(gomega.BeNil())
			g.Expect(tmpl.SchedulingPolicy.Gang).To(gomega.BeNil())
		}, timeout, interval).Should(gomega.Succeed())

		ginkgo.By("verifying a single Basic PodGroup exists, not one per ReplicatedJob")
		gomega.Eventually(func(g gomega.Gomega) {
			var pgList schedulingv1alpha3.PodGroupList
			g.Expect(k8sClient.List(ctx, &pgList, client.InNamespace(nsObj.Name))).To(gomega.Succeed())
			g.Expect(pgList.Items).To(gomega.HaveLen(1))
			pg := pgList.Items[0]
			g.Expect(pg.Name).To(gomega.HavePrefix(fmt.Sprintf("%s-", js.Name)))
	...
	ginkgo.It("should not create scheduling objects when JobSet is created suspended", func() {
		...
		js := makeSchedulingJobSet("sched-suspended", nsObj.Name,
			...
			&jobset.JobSetScheduling{
				SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
					Gang: &schedulingv1alpha3.WorkloadCompositePodGroupGangSchedulingPolicy{MinGroupCount: ptr.To[int32](4)},
				},
			},
		)
...
var _ = ginkgo.Describe("Workload-Aware Scheduling resume integration", func() {
	ginkgo.BeforeEach(func() {
		features.SetFeatureGateDuringTest(ginkgo.GinkgoTB(), features.JobSetWorkloadAwareSchedulingAPI, true)
	})

	ginkgo.It("recreates child Jobs when scheduling changes while suspended", func() {
		ctx := context.Background()
		nsObj := createTestNamespace(ctx, "sched-resume-change-")
		defer func() {
			gomega.Expect(testutil.DeleteNamespace(ctx, k8sClient, nsObj)).To(gomega.Succeed())
		}()

		js := makeSchedulingJobSet("sched-resume-change", nsObj.Name,
			[]jobset.ReplicatedJob{makeRJob("workers", 1, 1, 1)},
			&jobset.JobSetScheduling{SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
				Gang: &schedulingv1alpha3.WorkloadCompositePodGroupGangSchedulingPolicy{},
			}})
		gomega.Expect(k8sClient.Create(ctx, js)).To(gomega.Succeed())
		gomega.Eventually(func(g gomega.Gomega) {
			var jobs batchv1.JobList
			g.Expect(k8sClient.List(ctx, &jobs, client.InNamespace(nsObj.Name))).To(gomega.Succeed())
			g.Expect(jobs.Items).To(gomega.HaveLen(1))
		}, timeout, interval).Should(gomega.Succeed())

		var originalJob batchv1.Job
		var jobs batchv1.JobList
		gomega.Expect(k8sClient.List(ctx, &jobs, client.InNamespace(nsObj.Name))).To(gomega.Succeed())
		originalJob = jobs.Items[0]
		originalTemplate := originalJob.Annotations[controllers.SchedulingGroupTemplateNameKey]

		var current jobset.JobSet
		gomega.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(js), &current)).To(gomega.Succeed())
		current.Spec.Suspend = ptr.To(true)
		gomega.Expect(k8sClient.Update(ctx, &current)).To(gomega.Succeed())
		removePodGroupProtectionFinalizers(ctx, nsObj.Name)
		gomega.Eventually(func(g gomega.Gomega) {
			var pgs schedulingv1alpha3.PodGroupList
			g.Expect(k8sClient.List(ctx, &pgs, client.InNamespace(nsObj.Name))).To(gomega.Succeed())
			g.Expect(pgs.Items).To(gomega.BeEmpty())
		}, timeout, interval).Should(gomega.Succeed())

		gomega.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(js), &current)).To(gomega.Succeed())
		current.Spec.Scheduling = &jobset.JobSetScheduling{ReplicatedJobs: []jobset.ReplicatedJobScheduling{{
			TargetReplicatedJobs: []string{"workers"},
			SchedulingPolicy:     &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{Basic: &schedulingv1alpha3.WorkloadCompositePodGroupBasicSchedulingPolicy{}},
		}}}
		gomega.Expect(k8sClient.Update(ctx, &current)).To(gomega.Succeed())
		...
		gomega.Eventually(func(g gomega.Gomega) {
			var recreated batchv1.JobList
			g.Expect(k8sClient.List(ctx, &recreated, client.InNamespace(nsObj.Name))).To(gomega.Succeed())
			g.Expect(recreated.Items).To(gomega.HaveLen(1))
			g.Expect(recreated.Items[0].Annotations[controllers.SchedulingGroupTemplateNameKey]).NotTo(gomega.Equal(originalTemplate))
		}, timeout, interval).Should(gomega.Succeed())
	})
})

var _ = ginkgo.Describe("Workload-Aware Scheduling top-level Basic integration", func() {
	ginkgo.BeforeEach(func() {
		features.SetFeatureGateDuringTest(ginkgo.GinkgoTB(), features.JobSetWorkloadAwareSchedulingAPI, true)
	})

	ginkgo.It("creates one PodGroup for all ReplicatedJobs", func() {
		ctx := context.Background()
		nsObj := createTestNamespace(ctx, "sched-basic-top-level-")
		defer func() {
			gomega.Expect(testutil.DeleteNamespace(ctx, k8sClient, nsObj)).To(gomega.Succeed())
		}()

		js := makeSchedulingJobSet("sched-basic-top-level", nsObj.Name, []jobset.ReplicatedJob{
			makeRJob("driver", 1, 1, 1),
			makeRJob("workers", 2, 2, 2),
		}, &jobset.JobSetScheduling{
			SchedulingPolicy: &schedulingv1alpha3.WorkloadCompositePodGroupSchedulingPolicy{
		...
		gomega.Eventually(func(g gomega.Gomega) {
			workload := getWorkloadForJobSet(ctx, g, nsObj.Name)
			g.Expect(workload.Spec.PodGroupTemplates).To(gomega.HaveLen(1))
			g.Expect(workload.Spec.PodGroupTemplates[0].SchedulingPolicy.Basic).NotTo(gomega.BeNil())
		}, timeout, interval).Should(gomega.Succeed())
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(k8sClient.List(ctx, &pgList, client.InNamespace(nsObj.Name))).To(gomega.Succeed())
			g.Expect(pgList.Items).To(gomega.HaveLen(1))
			g.Expect(pgList.Items[0].Spec.SchedulingPolicy.Basic).NotTo(gomega.BeNil())
		}, timeout, interval).Should(gomega.Succeed())
	})
})
...
var _ = ginkgo.Describe("Workload-Aware Scheduling DRA ResourceClaims integration", func() {
	...
	ginkgo.It("should propagate resourceClaims from JobScheduling to per-Job PodGroups", func() {
		ctx := context.Background()
		...
		ginkgo.By("verifying Workload is created with per-Job PodGroupTemplates")
		gomega.Eventually(func(g gomega.Gomega) {
			workload := getWorkloadForJobSet(ctx, g, nsObj.Name)
			g.Expect(workload.Spec.PodGroupTemplates).To(gomega.HaveLen(5))

			var workerTemplateCount int
			...
			for i := range workload.Spec.PodGroupTemplates {
				...
				switch {
				case strings.HasPrefix(template.Name, "workers-"):
					workerTemplateCount++
					g.Expect(template.ResourceClaims).To(gomega.HaveLen(1))
					g.Expect(template.ResourceClaims[0].Name).To(gomega.Equal("imex-channel"))
					g.Expect(template.ResourceClaims[0].ResourceClaimTemplateName).To(gomega.HaveValue(gomega.Equal("imex-channel-template")))
				case strings.HasPrefix(template.Name, "driver-"):
					driverTemplate = template
				}
			}
			g.Expect(workerTemplateCount).To(gomega.Equal(4))
		...
		ginkgo.By("verifying a workers PodGroup carries the resourceClaims")
		gomega.Eventually(func(g gomega.Gomega) {
			workersPG := getPodGroupByPrefix(ctx, g, nsObj.Name, fmt.Sprintf("%s-workers-0", js.Name))

			// PodGroup should have the resourceClaims from the Job policy.
	...
	ginkgo.It("should propagate resourceClaims with direct ResourceClaimName reference", func() {
		...
		ginkgo.By("verifying a workers PodGroup carries the direct ResourceClaimName")
		gomega.Eventually(func(g gomega.Gomega) {
			workersPG := getPodGroupByPrefix(ctx, g, nsObj.Name, fmt.Sprintf("%s-workers-0", js.Name))
	...
	ginkgo.It("should propagate multiple resourceClaims to each per-Job PodGroup", func() {
		ctx := context.Background()
		...
		ginkgo.By("verifying a workers PodGroup has both resourceClaims and topology constraints")
		gomega.Eventually(func(g gomega.Gomega) {
			workersPG := getPodGroupByPrefix(ctx, g, nsObj.Name, fmt.Sprintf("%s-workers-0", js.Name))
