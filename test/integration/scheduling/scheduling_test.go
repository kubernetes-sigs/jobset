...
var _ = ginkgo.Describe("Workload-Aware Scheduling integration", func() {
	...
	ginkgo.It("should create one Basic PodGroup per ReplicatedJob when the top-level policy is Basic", func() {
		...
		// top-level Basic policy with no replicatedJobPolicies overrides falls
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			for _, pg := range pgList.Items {
				g.Expect(pg.Name).To(gomega.SatisfyAny(
					gomega.Equal(fmt.Sprintf("%s-driver", js.Name)),
					gomega.Equal(fmt.Sprintf("%s-workers", js.Name)),
				))
	...
	ginkgo.It("should create one PodGroup per Job when jobSchedulingPolicy is set (Gang-of-Gangs per-Job model)", func() {
		...
		// directly), while "worker" uses jobSchedulingPolicy so that each of its
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(templateNames).To(gomega.ConsistOf("launcher", "worker-0", "worker-1"))
		}, timeout, interval).Should(gomega.Succeed())
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			for _, job := range jobList.Items {
				// The per-Job PodGroup name is derived the same way as the Job's
				// own name, so they must be identical.
				var pg schedulingv1alpha3.PodGroup
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{
					Name: job.Name, Namespace: nsObj.Name,
				}, &pg)).To(gomega.Succeed())
		...
		gomega.Eventually(func(g gomega.Gomega) {
			var launcherPG schedulingv1alpha3.PodGroup
			g.Expect(k8sClient.Get(ctx, types.NamespacedName{
				Name: fmt.Sprintf("%s-launcher", js.Name), Namespace: nsObj.Name,
			}, &launcherPG)).To(gomega.Succeed())
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			for _, job := range jobList.Items {
				...
				g.Expect(templateName).To(gomega.SatisfyAny(gomega.Equal("worker-0"), gomega.Equal("worker-1")))
				seen[templateName] = true

				// The pod template's schedulingGroup should reference this
				// Job's own dedicated PodGroup, matching the Job's own name.
				...
				g.Expect(*job.Spec.Template.Spec.SchedulingGroup.PodGroupName).To(gomega.Equal(job.Name))
			}
	...
	ginkgo.It("should patch Gang minCount in place when ElasticJobSet changes parallelism", func() {
		...
		var originalPodGroup schedulingv1alpha3.PodGroup
		gomega.Expect(k8sClient.Get(ctx, types.NamespacedName{
			Name: js.Name, Namespace: nsObj.Name,
		}, &originalPodGroup)).To(gomega.Succeed())
