...
var _ = ginkgo.Describe("JobSet controller", func() {
	...
	ginkgo.It("JobSet with VolumeClaimPolicies should create, retain, and reuse PVCs", func() {
		ctx := context.Background()
		...
		// Create first JobSet with two PVCs (Delete and Retain policies)
		ginkgo.By("creating first JobSet with Delete and Retain PVC policies")
		js1 := testing.MakeJobSet("volume-test", namespace).
			SuccessPolicy(&jobset.SuccessPolicy{Operator: jobset.OperatorAll}).
			EnableDNSHostnames(true).
			VolumeClaimPolicies([]jobset.VolumeClaimPolicy{
				{
		...
		ginkgo.By("verifying PVCs are created")
		...
		gomega.Eventually(testutil.NumJobs, timeout, interval).
			WithArguments(ctx, k8sClient, js1).
			Should(gomega.Equal(testutil.NumExpectedJobs(js1)))

		ginkgo.By("completing all jobs")
		var jobList batchv1.JobList
		gomega.Expect(k8sClient.List(ctx, &jobList, client.InNamespace(namespace))).To(gomega.Succeed())
		completeAllJobs(&jobList)

		ginkgo.By("verifying JobSet completes")
		testutil.JobSetCompleted(ctx, k8sClient, js1, timeout)

		ginkgo.By("deleting the first JobSet")
		gomega.Expect(k8sClient.Delete(ctx, js1)).To(gomega.Succeed())
		...
		js2 := testing.MakeJobSet("volume-test-2", namespace).
			SuccessPolicy(&jobset.SuccessPolicy{Operator: jobset.OperatorAll}).
			VolumeClaimPolicies([]jobset.VolumeClaimPolicy{
				{
					Templates: []corev1.PersistentVolumeClaim{
		...
		gomega.Eventually(testutil.NumJobs, timeout, interval).
			WithArguments(ctx, k8sClient, js2).
			Should(gomega.Equal(testutil.NumExpectedJobs(js2)))
