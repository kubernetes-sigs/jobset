...
type JobSetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.JobSet, err error)
...
// List lists all JobSets in the indexer.
func (s *jobSetLister) List(selector labels.Selector) (ret []*v1.JobSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.JobSet))
	})
	return ret, err
}
...
type JobSetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.JobSet, error)
...
// List lists all JobSets in the indexer for a given namespace.
func (s jobSetNamespaceLister) List(selector labels.Selector) (ret []*v1.JobSet, err error) {
...
// Get retrieves the JobSet from the indexer for a given namespace and name.
func (s jobSetNamespaceLister) Get(name string) (*v1.JobSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("jobset"), name)
	}
	return obj.(*v1.JobSet), nil
}
