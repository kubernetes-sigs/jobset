...
- [Alternatives](#alternatives)
  ...
  - [Using Separate CRD](#using-separate-crd)
  - [Integration with Kueue's ClusterQueue Directly](#integration-with-kueues-clusterqueue-directly)
<!-- /toc -->
...
2. **Ownership and Lifecycle**:
   ...
   - The Workload name follows the pattern: `<jobset-name>-workload`
...
4. **Status Synchronization**:
   - JobSet status is updated based on both Job status and Workload admission status
   - New condition: `WorkloadAdmitted` indicates when the gang has been admitted for scheduling
...
Users will be able to check gang scheduling status via JobSet conditions:
...
status:
  ...
  - type: WorkloadAdmitted
    ...
    message: "Waiting for 64 GPU resources to become available"
    lastTransitionTime: "2025-12-21T10:00:00Z"
```

When the Workload is admitted:
```yaml
status:
  conditions:
  - type: WorkloadAdmitted
    status: "True"
    reason: "Admitted"
    message: "Gang has been admitted for scheduling"
    lastTransitionTime: "2025-12-21T10:15:00Z"
```

Events will be emitted for key gang scheduling milestones:
- `WorkloadCreated`: When the Workload object is created
- `WorkloadAdmitted`: When the scheduler admits the gang
- `WorkloadRejected`: If the scheduler rejects the gang (e.g., insufficient quota)
...
**Beta** (v0.7.0):
- JobSetWorkloadTemplate mode fully implemented
- Tested with multiple schedulers (Kueue, Volcano)
- Production usage feedback incorporated
- Metrics for gang scheduling performance
- Comprehensive user documentation and examples

**Stable** (v1.0.0):
- At least 2 releases of beta stability
- Multiple production deployments
- Performance benchmarks published
- Migration guide for users of workarounds
...
We could create a separate GangSchedulingPolicy CRD that references JobSets.

**Rejected because**:
- Increases operational complexity (two objects to manage)
- Less intuitive user experience
- Harder to maintain ownership and lifecycle management
- The gang policy is inherently tied to JobSet lifecycle

### Integration with Kueue's ClusterQueue Directly

We could skip the Workload API and integrate directly with Kueue's ClusterQueue.

**Rejected because**:
- Locks JobSet to a single scheduler implementation
- Violates Kubernetes extensibility principles
- The Workload API is the standard abstraction for gang scheduling
- Reduces portability to other schedulers
