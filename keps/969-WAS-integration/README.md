# KEP-WAS: Workload-Aware Scheduling Integration
...
| Scale an elastic workload | Changes to the represented pod count cause immutable Workload and PodGroup resources to be recreated. |
| Share a DRA claim across a Job's pods | A `resourceClaims` entry on a ReplicatedJob policy is copied to that ReplicatedJob's PodGroup. Pod-level claims remain independent. |
...
| Workload and PodGroup specs are immutable | Detect drift and delete/recreate owned resources. |
...
1. **Top-level mode** is selected when the effective composite policy is Gang, there are no
   targeted overrides, and startup is not sequenced. One WorkloadItem represents the entire
   JobSet.
2. **Per-ReplicatedJob mode** is selected when overrides exist or startup is sequenced. One
   ...
   composite defaults.
3. Each WorkloadItem is compiled with `workloadbuilder`. Because the current builder supports
   only single-item trees, per-ReplicatedJob templates are merged into one Workload.
4. The controller creates one PodGroup for each resulting template and sets the JobSet as the
   controller owner of the Workload and PodGroups.
5. Child Jobs receive:
   - `scheduling.k8s.io/group-template-name`, identifying the template;
   - `scheduling.k8s.io/parent-composite-podgroup`, identifying the JobSet; and
...
Workload and PodGroup specs are immutable. When ElasticJobSet scaling or another supported
change alters a generated template, the controller deletes the owned Workload and PodGroups and
...
| Generated spec changes | Delete and recreate immutable resources. |
...
- **Integration:** Workload/PodGroup creation, ownership, suspend/resume, scaling, DRA claim
  propagation, and gate-disabled behavior.
- **E2E:** top-level Gang, per-ReplicatedJob policies, suspension, sequencing, preemption, and
  scaling on a WAS-enabled cluster.
